...
def infer_investigation_suggestions(df: pd.DataFrame | None) -> list[str]:
    ...
    if has("Sales") and has("Ship Mode"):
        suggestions.append("Как режим доставки связан с объемом продаж?")
    if has("Sales") and has("Segment"):
        suggestions.append("Какие сегменты клиентов дают наибольшие продажи?")
    if has("Sales") and has("Category"):
        suggestions.append("Какие категории товаров дают наибольшие продажи?")
    if has("Sales") and has("Region"):
        suggestions.append("В каких регионах продажи выше или ниже среднего?")
    if has("Sales") and has("City"):
        suggestions.append("Какие города дают наибольший вклад в продажи?")
    if has("Order Date") and has("Sales"):
        suggestions.append("Как менялись продажи по датам заказов?")
    ...
    with left:
        ...
        if shareable_reports:
            ...
            with st.container(border=True):
                ...
                if st.button(
                    "Archive report",
                    key=f"archive-share-{selected_report.report_id}",
                    disabled=selected_report.status.value == "archived",
                    width="stretch",
                ):
                    store.archive_shareable_report(selected_report.report_id)
                    st.rerun()
...
with st.sidebar:
    ...
    with st.form("new_investigation_form", clear_on_submit=True):
        suggestions = infer_investigation_suggestions(st.session_state.workspace_df)
